...
「未調査」と「調査済みだが引用なし」は区別しています。未調査の記事を、引用を持たない一次資料とは判定しません。ノードをクリックすると詳細カードが固定され、カード内のリンクを操作できます。
...
(function () {
  ...
  var W,H,nodes=[],edges=[],hovered=null,selected=null;
  ...
  function draw(){ctx.clearRect(0,0,W,H);var by={};nodes.forEach(function(n){by[n.id]=n;});edges.forEach(function(e){if(by[e.source]&&by[e.target])drawArrow(by[e.source],by[e.target],e);});nodes.forEach(function(n){ctx.fillStyle=n.external?"#8c959f":"#0969da";ctx.beginPath();ctx.arc(n.x,n.y,n.r,0,Math.PI*2);ctx.fill();if(n===hovered||n===selected){ctx.strokeStyle="#1f2328";ctx.lineWidth=n===selected?3:2;ctx.stroke();}ctx.fillStyle="#1f2328";ctx.font="11px sans-serif";ctx.textAlign="center";ctx.fillText(n.title.slice(0,28),n.x,n.y+n.r+13);});}
  ...
  function filter(week){
    ...
    selected=null;hovered=null;show(null);layout();draw();
  }
  function pick(ev){var r=cv.getBoundingClientRect(),x=ev.clientX-r.left,y=ev.clientY-r.top;return nodes.find(function(v){var dx=x-v.x,dy=y-v.y;return dx*dx+dy*dy<(v.r+6)*(v.r+6);})||null;}
  cv.addEventListener("mousemove",function(ev){var n=pick(ev);if(n!==hovered){hovered=n;show(selected||n);draw();}});
  cv.addEventListener("click",function(ev){var n=pick(ev);selected=selected===n?null:n;show(selected||hovered);draw();});
