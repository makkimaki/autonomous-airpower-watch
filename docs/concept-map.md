...
ニュースに登場する用途領域、プログラム、企業、機体、世界モデル、シミュレータ、技術・政策トピックの関係を可視化したマップです。
ノードの大きさは掲載件数、線の太さは同じニュースに登場した回数を表します。ドラッグで移動、クリックで詳細を表示します。

この線は引用、因果、技術的依存を表しません。記事単位の参照関係は[引用・情報伝播グラフ](./citations.md)で確認できます。
...
<script>
(function () {
  var GROUPS = ["domain", "program", "company", "aircraft", "model", "simulator", "topic"];
  var GROUP_LABELS = { domain: "用途領域", program: "プログラム", company: "企業", aircraft: "機体", model: "世界モデル", simulator: "シミュレータ", topic: "技術・政策" };
  var COLORS = ["#0969da", "#8250df", "#1a7f37", "#bf8700", "#cf222e", "#0a7b83", "#57606a"];
  ...
  function init(DATA) {
    ...
    GROUPS.forEach(function (g) {
      ...
      b.appendChild(d);
      b.appendChild(document.createTextNode(GROUP_LABELS[g]));
      b.addEventListener("click", function () {
        focusGroup = focusGroup === g ? null : g;
        legend.querySelectorAll("button").forEach(function (x) {
          x.setAttribute("aria-pressed", String(x.textContent === (GROUP_LABELS[focusGroup] || "")));
        });
        draw();
      });
    ...
    function showCard(n) {
      ...
      card.innerHTML =
        ...
        '"></span>' + esc(GROUP_LABELS[n.group] || n.group) + "</div><h3>" + esc(n.label) + "</h3>" +
